...
const Audit = require('../../../audits/accessibility/link-in-text-block.js');
const assert = require('assert');

/* eslint-env mocha */

describe('Accessibility: link-in-text-block audit', () => {
  ...
  it('doesn\'t throw an error when violations is undefined', () => {
    ...
    const output = Audit.audit(artifacts);
    assert.equal(output.description, 'Links can be distinguished without relying on color');
  });
});
